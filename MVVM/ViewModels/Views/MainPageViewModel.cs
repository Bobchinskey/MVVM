    ...
    class MainPageViewModel : ViewModelBase
    {
        ...
        public MainPageViewModel()
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                NewsAdapter.SelectCommand = new SqlCommand("select Top(5) [id_news],[date_publication],[heading],[news] from [News] ORDER BY [id_news] desc", connection);
                NewsAdapter.Fill(DataTableBaseNewsPresenter);
                ImportantInformationAdapter.SelectCommand = new SqlCommand("select Top(5) [id_important_information],[date_publication],[heading],[important_information] from [Important_information] ORDER BY [id_important_information] desc", connection);
